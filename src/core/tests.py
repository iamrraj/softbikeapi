...
class EndpointsTest(TestCase):
    @classmethod
    def setUpClass(cls):
        cls.user = User(username='testuser', first_name='Test', last_name='User')
        cls.user.set_password('testpassword')
        cls.user.save()
        cls.app = Application.objects.create(client_id='testapp', client_type='public', authorization_grant_type='password', client_secret='testsecret', skip_authorization=True)
    ...
    def test_data(self):
        ...
        self.assertTrue(DataBatch.objects.filter(uuid='test', user=self.user).exists())
    ...
    def test_push(self):
        ...
        self.assertTrue(PushToken.objects.filter(user=self.user, token='a-token').exists())
