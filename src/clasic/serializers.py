...
class ClassicSerializer(serializers.ModelSerializer):
    class Meta:
        model = ClassicBike
        fields =('pk','bikeid','milage','movingtime','averagespeed','kgtrasported','co2','additionalbox','too','fromm','letteritems','shipweight','package')
...
class DDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ClassicBike
        fields = ('pk','bikeid','milage','movingtime','averagespeed','kgtrasported','co2','additionalbox','too','fromm','letteritems','shipweight','package','items')
...
class SummerySerializer(serializers.ModelSerializer):
    ...
    def get_total_letter(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_letter=Sum('letteritems'))
        return totalpieces["total_letter"]

    def get_total_ship_weight(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_ship_weight=Sum('shipweight'))
        return totalpieces["total_ship_weight"]

    def get_total_pack(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_pack=Sum('package'))
        return totalpieces["total_pack"]


    def get_total_milage(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_milage=Sum('milage'))
        return totalpieces["total_milage"]
    
    def get_total_movingtime(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_movingtime=Sum('movingtime'))
        return totalpieces["total_movingtime"]
    
    def get_total_averagespeed(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_averagespeed=Round(Avg('averagespeed')))
        return totalpieces["total_averagespeed"]

    def get_total_kg(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_kg=Sum('kgtrasported'))
        return totalpieces["total_kg"]
    
    def get_total_co2_save(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_co2_save=Sum('co2'))
        return totalpieces["total_co2_save"]
    
    def get_total_boxes(self, obj):
        totalpieces = ClassicBike.objects.all().aggregate(total_boxes=Sum('additionalbox'))
...
class DSummerySerializer(serializers.ModelSerializer):
    ...
    def get_total_letter(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_letter=Sum('letteritems'))
        return totalpieces["total_letter"]

    def get_total_ship_weight(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_ship_weight=Sum('shipweight'))
        return totalpieces["total_ship_weight"]
    ...
    def get_total_milage(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_milage=Sum('milage'))
        return totalpieces["total_milage"]
    
    def get_total_movingtime(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_movingtime=Sum('movingtime'))
        return totalpieces["total_movingtime"]
    
    def get_total_averagespeed(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_averagespeed=Round(Avg('averagespeed')))
        return totalpieces["total_averagespeed"]

    def get_total_kg(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_kg=Sum('kgtrasported'))
        return totalpieces["total_kg"]

    def get_total_boxes(self, obj):
        totalpieces = Cdetails.objects.all().aggregate(total_boxes=Sum('additionalbox'))
        return totalpieces["total_boxes"]
